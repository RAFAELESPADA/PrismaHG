...
public class Grappler extends Kit {
	...
	public Grappler() {
		...
		/* 351 */         indiob.add(ChatColor.GRAY + "Use sua corda");
		indiob.add(ChatColor.GRAY + "Para andar por ai");
				setDescrição(indiob);
		
		setItens(new ItemBuilder().material(Material.LEASH).name(getItemColor() + "Kit " + getNome()).build());
	}
	...
	@EventHandler
	public void removeCorda1(PlayerMoveEvent e) {
	...
	public void onLeash(PlayerLeashEntityEvent e) {
		if (containsHability(e.getPlayer())) {
	 		e.setCancelled(true);
	...
	public void usar(PlayerInteractEvent e) {
		Player p = e.getPlayer();
	 	if ((p.getItemInHand().getType().equals(Material.LEASH)) && (useAbility(p))) {
	 	     e.setCancelled(true);
